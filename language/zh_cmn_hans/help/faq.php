...
$lang = array_merge($lang, array(
	...
	'HELP_FAQ_FRIENDS_BASIC_ANSWER'	=> '您可以使用这些名单来管理论坛中的用户。加入好友名单的用户会在您的用户控制面板中列出，方便您快速查看在线状态和发送站内消息。另外如果有模板支持，您的好友所发表的文章也会高亮显示。如果您将某个用户拉黑，他的帖子将对您自动隐藏。',
	...
	'HELP_FAQ_GROUPS_ADMINISTRATORS_ANSWER'	=> '管理员是整个论坛中拥有最高权限的用户。管理员用户可以从各个方面管理论坛，包括设置权限，封禁用户，创建用户组或版主等等。取决于论坛创建者赋予的权限，他们还可以拥有所有版面的版主权限。',
	...
	'HELP_FAQ_GROUPS_MODERATORS_ANSWER'	=> '版主可以是个人或用户组，他们的工作是维护每个版面的日常运作。他们拥有在所辖版面内的编辑/删除/锁定/解锁/移动/分割主题和投票的权力。一般版主会阻止用户发表与版面无关的文章，垃圾文章或有冒犯内容的文章。',
	...
	'HELP_FAQ_GROUPS_USERGROUPS_ANSWER'	=> '用户组是论坛用于管理用户的一种方式。每个用户可以属于多个用户组 (这与其他的论坛软件不太一样) 并且每个用户组可以设置独立的权限。这使得管理员可以很方便的同时管理多个用户的权限，例如版主的权限, 使其可以访问私人版面等等。',
	'HELP_FAQ_GROUPS_USERGROUPS_JOIN_ANSWER'	=> '要加入一个用户组请点击用户控制面板中的用户组链接，这样您可以看到所有的用户组。并不是所有的用户组都是 <em>开放</em> 的，一些需要申请，一些是关闭的，而另一些甚至是隐藏的。如果是开放的用户组您可以直接点击加入。如果是需要申请的用户组，您需要先申请加入，而后等待用户组领导者批准后才能成为用户组的成员，这个过程中他们可能会询问您加入的原因。如果申请没有被批准，请不要纠缠用户组长，他们有拒绝您加入的理由。',
	'HELP_FAQ_GROUPS_USERGROUPS_JOIN_QUESTION'	=> '我该如何加入一个用户组？',
	'HELP_FAQ_GROUPS_USERGROUPS_LEAD_ANSWER'	=> '当用户组被论坛管理员创建时, 会同时设置用户组的组长。如果您有兴趣新建一个用户组，请先通过站内消息联系论坛管理员。',
	'HELP_FAQ_GROUPS_USERGROUPS_LEAD_QUESTION'	=> '我该如何成为一个用户组长',
	...
	'HELP_FAQ_ISSUES_LEGAL_ANSWER'	=> '您应该联络这个论坛的管理员。通过“论坛团队”名单您可以看到论坛的管理员名单。如果投诉没有收到回复，您应该联络这个域名的所有者 (使用 <a href="http://www.google.com/search?q=whois">whois lookup</a>)或者, 如果这个论坛运行于一个免费的服务器 (例如 yahoo, free.fr, f2s.com, 等等), 联络管理者或者服务商的违规管理部门。请注意 phpBB 团队<strong>绝对没有</strong>控制并且无论如何没有相关的责任和义务来管理使用这个论坛的用户行为。对 phpBB 开发组发布任何与 phpbb.com 网站<strong>没有直接关系</strong>的法律声明 (服务中断, 连带责任, 诽谤等等) 都是没有任何意义的。如果您给 phpBB 团队发送任何<strong>关于第三方</strong>使用此软件的信件，都有可能得到简短的声明或不予回复。',
	'HELP_FAQ_ISSUES_LEGAL_QUESTION'	=> '对于论坛上诽谤, 滥用以及其他涉及法律的事务, 我该联络谁？',
	...
	'HELP_FAQ_LOGIN_CANNOT_LOGIN_ANSWER'	=> '这种情况有好几种原因。首先确认您的用户名和密码是否正确。如果是，请联络论坛的管理员确认是否封禁了您的帐号。也有可能是网站的管理员在后台进行了错误的设置，请联络他们修复错误。',
	'HELP_FAQ_LOGIN_CANNOT_LOGIN_ANYMORE_ANSWER'	=> '最可能的原因是, 您输入了一个错误的用户名或密码(检查一下注册时发给您的email)。也可能出于某些原因管理员封禁或删除了您的帐号。如果是后一种, 也许是您很长一段时间没有发表任何文章? 某些论坛为了控制数据库大小会定期清理长期不活动的用户。如果是这种情况，请重新注册一个帐号并积极参与讨论吧。',
	'HELP_FAQ_LOGIN_CANNOT_LOGIN_ANYMORE_QUESTION'	=> '我注册过帐号但是现在却无法登录了？！',
	'HELP_FAQ_LOGIN_CANNOT_LOGIN_QUESTION'	=> '为什么我不能登录？',
	'HELP_FAQ_LOGIN_CANNOT_REGISTER_ANSWER'	=> '这可能是因为网站管理者禁止了新用户注册，也可能是因为网站管理者封禁了您所在的IP地址或禁止了您试图注册的用户名。需要更多的帮助请联络论坛管理员。',
	...
	'HELP_FAQ_LOGIN_REGISTER_ANSWER'	=> '您不一定必须注册，这取决于论坛管理员设置是否需要您注册后才能发表帖子。但是注册将给您更多的权限来使用其他的论坛功能，例如自定义头像, 站内短信, 给好友发送email和加入用户组等等。只需要一小会儿就能完成注册，所以建议您还是注册一个帐号吧。',
	...
	'HELP_FAQ_PMS_CANNOT_SEND_ANSWER'	=> '有三个可能的原因：您还未注册或还未登录，论坛管理员禁止了站内短信，或者论坛管理员取消了您的站内短信权限。请联络管理员询问其原因。',
	...
	'HELP_FAQ_PMS_UNWANTED_ANSWER'	=> '您可以在用户控制面板中通过设定站内消息接收规则来拒绝来自某个用户的消息。如果您收到类似的短信，请向管理员投诉，他们有权限禁止这些用户发送站内短信。',
	...
	'HELP_FAQ_POSTING_CREATE_ANSWER'	=> '在版面上发表文章, 请点击相应的按钮。您可能需要注册之后才能发表文章，您所拥有的权限列表显示在版面和文章页面的下方。例如：您可以发表新主题, 您可以添加附件, 等等。',
	'HELP_FAQ_POSTING_CREATE_QUESTION'	=> '我该如何在版面上发表话题或发表回复？',
	...
	'HELP_FAQ_POSTING_NO_ATTACHMENTS_ANSWER'	=> '发表附件的权限, 可以通过版面/用户/用户组来设置。论坛管理员可能不允许在某些版面粘贴附件，或者只允许某些用户组发表附件。如果您不清楚为什么不能发表附件，请联络论坛管理员。',
	...
	'HELP_FAQ_POSTING_QUEUE_ANSWER'	=> '论坛管理员可能设置了这个版面的帖子必须通过审批才能发表。这也可能是因为您被管理员放入了需要审批文章的用户组。请联络论坛管理员以得到更多信息。',
	...
	'HELP_FAQ_SEARCH_FORUM_ANSWER'	=> '在首页或版面文章列表页, 文章浏览页的搜索栏中输入关键字就可以查找. 点击每页页首的 “搜索” 链接将进入高级搜索。具体的位置因模板而异。',
