...
$lang = array_merge($lang, array(
	...
	'ALLOW_MASS_PM'				=> '允许对多个用户和群组发送私人短信',
	...
	'PM_MAX_RECIPIENTS_EXPLAIN'   => '对单个私人短信的收信人数量进行限制. 如果设置为 0 将不作限制. 此设置可以在群组设置页面中对每个群组单独进行设置.',
));
...
$lang = array_merge($lang, array(
	...
	'NEW_MEMBER_POST_LIMIT_EXPLAIN'	=> '新用户会被归入 <em>新注册用户</em> 组直到其发表足够多的文章. 您可以使用此群组的组权限来限制其使用站内短信以及方便发文审核. <strong>设置为0则不启用此项功能.</strong>',
	'NEW_MEMBER_GROUP_DEFAULT'		=> '设置新注册群组为默认组',
	'NEW_MEMBER_GROUP_DEFAULT_EXPLAIN'	=> '如果设置为是, 并且设定了新用户发帖数目限制, 新用户不仅会加入到 <em>新注册用户</em> 组, 这个群组也会成为他的默认组. 这将方便管理员为该组指定一个默认等级和组头像.',
